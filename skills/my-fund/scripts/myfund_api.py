...
def load_runtime_env() -> RuntimeEnv:
    ...
    return RuntimeEnv(values=values, dotenv_path=dotenv_path, dotenv_found=dotenv_path.exists())


def config_source_hint(runtime_env: RuntimeEnv) -> str:
    if runtime_env.dotenv_found:
        return (
            "The skill-local .env file was found, but the required variable is missing. "
            "You can also provide the value as an exported environment variable."
        )
    return (
        "The skill-local .env file was not found. Create it from .env.example or provide the value "
...
def require_portfolio(config: RuntimeConfig) -> str:
    if not config.portfolio:
        raise MyFundError(
            "Missing portfolio. Pass --portfolio, export MYFUND_PORTFEL, or define MYFUND_PORTFEL in the skill-local .env. MYFUND_PORTFOLIO is also accepted for compatibility."
        )
...
def normalize_payload(payload: dict[str, Any], config: RuntimeConfig) -> dict[str, Any]:
    ...
    normalized_payload = {
        ...
        "status": {
            "code": str((payload.get("status") or {}).get("code", "")),
            "ok": str((payload.get("status") or {}).get("code", "")) == "0",
            "text": (payload.get("status") or {}).get("text"),
        },
...
def slugify_portfolio_name(name: str | None) -> str:
    source = (name or "portfolio").strip().lower()
...
def build_derived_payload(normalized_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    holdings_sorted = sorted(
        ...
        key=lambda item: (
            item.get("wartosc") if isinstance(item.get("wartosc"), (int, float)) else 0.0
        ),
        reverse=True,
    )

    def _share_of_total(value: Any) -> float:
        if not isinstance(value, (int, float)) or not isinstance(total_value, (int, float)) or not total_value:
            return 0.0
        ...
        allocation_by_type.append(
            {
                ...
                "share_pct": _share_of_total(value),
            }
        )
    allocation_by_type.sort(key=lambda item: item["value"] if isinstance(item["value"], (int, float)) else 0.0, reverse=True)

    allocation_by_security = []
    for label, share_pct in (allocations.get("by_security") or {}).items():
        allocation_by_security.append(
            {
                "label": label,
                "share_pct": share_pct,
...
def _build_history_slice(timeseries: dict[str, Any]) -> dict[str, Any]:
    ...
    for day in sorted(set(portfolio_return_series.keys()) | set(benchmark_series.keys())):
        port_value = portfolio_return_series.get(day)
        bench_value = benchmark_series.get(day)
        portfolio_return.append(port_value)
        benchmark_return.append(bench_value)
        if isinstance(port_value, (int, float)) and isinstance(bench_value, (int, float)):
            benchmark_delta.append(round(port_value - bench_value, 2))
        else:
    ...
    return {
        ...
        "portfolio_return": [
            portfolio_return_series.get(day)
            for day in sorted(set(portfolio_return_series.keys()) | set(benchmark_series.keys()))
        ],
        "benchmark_return": [
            benchmark_series.get(day)
            for day in sorted(set(portfolio_return_series.keys()) | set(benchmark_series.keys()))
        ],
...
def _build_month_to_date_slice(timeseries: dict[str, Any]) -> dict[str, Any]:
    ...
    value_lookup = {day: value for day, value in zip(full["dates"], full["value"])}
    capital_lookup = {day: value for day, value in zip(full["dates"], full["capital"])}
    profit_lookup = {day: value for day, value in zip(full["dates"], full["profit"])}
    portfolio_return_lookup = {
        day: value for day, value in zip(full["return_dates"], full["portfolio_return"])
    }
    benchmark_return_lookup = {
        day: value for day, value in zip(full["return_dates"], full["benchmark_return"])
    }
    benchmark_delta_lookup = {
        day: value for day, value in zip(full["return_dates"], full["benchmark_delta"])
    }
